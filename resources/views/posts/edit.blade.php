@extends('layouts.app')
@section('title', 'Edit post | Homework')
    ...
    <div class="relative md:ml-64 bg-blueGray-50 min-h-full w-full">
        <div class="px-4 md:px-10 mx-auto w-full py-10">
            <div class="flex justify-between align-middle pb-4">
                <h1 class="block text-2xl font-bold uppercase mb-0">{{ __( 'Edit post' ) }}</h1>
            </div>
            <hr class="mb-4">
            <div class="mb-6">
            ...
            <div class="relative w-2/3  break-words bg-white w-full mb-6 shadow-xl rounded-lg p-4">
                <form action="/dashboard/posts/{{ $post->id  }}" method="POST">
                    ...
                    <div class="text-center mt-6">
                        <button
                            class="inline-block bg-gray-900 text-white active:bg-gray-700 text-sm font-bold uppercase px-6 py-3 rounded shadow hover:shadow-lg outline-none focus:outline-none mr-1 mb-1"
                            type="submit"
                        >
                            {{ __( 'Update post' ) }}
                        </button>
